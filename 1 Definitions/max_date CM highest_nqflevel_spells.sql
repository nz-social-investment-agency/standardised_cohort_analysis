...
DROP VIEW IF EXISTS [DL-MAA2023-46].[max_date_CM_highest_nqflevel_spells_202506]
GO

CREATE VIEW [DL-MAA2023-46].[max_date_CM_highest_nqflevel_spells_202506] AS
WITH uid_list AS (
...
count_each_date AS (
	...
	SELECT CAST([nqf_attained_date] AS DATE) AS the_date
		, COUNT(*) AS num
	FROM [IDI_Community].[edu_highest_nqflevel_spells].[highest_nqflevel_spells_202506]
	WHERE [nqf_attained_date] BETWEEN DATEADD(YEAR, -5, GETDATE()) AND GETDATE() -- events between now and five years ago
	GROUP BY CAST([nqf_attained_date] AS DATE) -- remove times if DATETIME
