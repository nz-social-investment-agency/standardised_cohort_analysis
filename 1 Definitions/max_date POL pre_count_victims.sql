...
DROP VIEW IF EXISTS [DL-MAA2023-46].[max_date_POL_pre_count_victimisations_202506]
GO

CREATE VIEW [DL-MAA2023-46].[max_date_POL_pre_count_victimisations_202506] AS
WITH uid_list AS (
...
count_each_date AS (
	...
	SELECT CAST([pol_prv_reported_date] AS DATE) AS the_date
		, COUNT(*) AS num
	FROM [IDI_Clean_202506].[pol_clean].[pre_count_victimisations]
	WHERE [pol_prv_reported_date] BETWEEN DATEADD(YEAR, -5, GETDATE()) AND GETDATE() -- events between now and five years ago
	GROUP BY CAST([pol_prv_reported_date] AS DATE) -- remove times if DATETIME
