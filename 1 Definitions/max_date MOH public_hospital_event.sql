...
DROP VIEW IF EXISTS [DL-MAA2023-46].[max_date_MOH_pub_fund_hosp_discharges_event_202506]
GO

CREATE VIEW [DL-MAA2023-46].[max_date_MOH_pub_fund_hosp_discharges_event_202506] AS
WITH uid_list AS (
...
count_each_date AS (
	...
	SELECT CAST([moh_evt_evst_date] AS DATE) AS the_date
		, COUNT(*) AS num
	FROM [IDI_Clean_202506].[moh_clean].[pub_fund_hosp_discharges_event]
	WHERE [moh_evt_evst_date] BETWEEN DATEADD(YEAR, -5, GETDATE()) AND GETDATE() -- events between now and five years ago
	GROUP BY CAST([moh_evt_evst_date] AS DATE) -- remove times if DATETIME
