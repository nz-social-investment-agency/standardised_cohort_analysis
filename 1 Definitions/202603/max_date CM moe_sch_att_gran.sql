...
DROP VIEW IF EXISTS [$(PROJECT_SCHEMA)].[max_date_CM_moe_sch_att_gran_$(REFRESH)]
GO

CREATE VIEW [$(PROJECT_SCHEMA)].[max_date_CM_moe_sch_att_gran_$(REFRESH)] AS
WITH uid_list AS (
...
count_each_date AS (
	...
	SELECT CAST(DATEFROMPARTS([year],12,31) AS DATE) AS the_date
		, COUNT(*) AS num
	FROM [IDI_Community].[edu_sch_att_gran].[sch_att_gran_$(REFRESH)]
	WHERE DATEFROMPARTS([year],12,31) BETWEEN DATEADD(YEAR, -5, GETDATE()) AND GETDATE() -- events between now and five years ago
	GROUP BY CAST(DATEFROMPARTS([year],12,31) AS DATE) -- remove times if DATETIME
