...
Outputs:
- [$(PROJECT_DB)].[$(PROJECT_SCHEMA)].[defn_mha_bipolar]
...
DROP TABLE IF EXISTS [$(PROJECT_DB)].[$(PROJECT_SCHEMA)].[tmp_bipolar_solo]
GO

CREATE TABLE [$(PROJECT_DB)].[$(PROJECT_SCHEMA)].[tmp_bipolar_solo] (
	snz_uid	INT,
	event_date DATE,
)

/* Diagnosis or treatment used for bipolar and other conditions */
DROP TABLE IF EXISTS [$(PROJECT_DB)].[$(PROJECT_SCHEMA)].[tmp_bipolar_multi]
GO

CREATE TABLE [$(PROJECT_DB)].[$(PROJECT_SCHEMA)].[tmp_bipolar_multi] (
	snz_uid	INT,
	event_date DATE,
)
...
INSERT INTO [$(PROJECT_DB)].[$(PROJECT_SCHEMA)].[tmp_bipolar_solo] (snz_uid, event_date)
...
INSERT INTO [$(PROJECT_DB)].[$(PROJECT_SCHEMA)].[tmp_bipolar_multi] (snz_uid, event_date)
SELECT a.[snz_uid]
		,[moh_pha_dispensed_date] AS event_date
FROM [IDI_Clean_$(REFRESH)].[moh_clean].[pharmaceutical] AS a
INNER JOIN [IDI_Metadata_$(REFRESH)].[moh_pharm].[dim_form_pack_subsidy_code] AS b
...
FROM [IDI_Clean_$(REFRESH)].[moh_clean].[lab_claims]
WHERE EXISTS (
	SELECT 1
	FROM [$(PROJECT_DB)].[$(PROJECT_SCHEMA)].[ref_diagnosis] AS r
...
/* Add indexes */
CREATE NONCLUSTERED INDEX my_index_name ON [$(PROJECT_DB)].[$(PROJECT_SCHEMA)].[tmp_bipolar_solo] (snz_uid);
GO
CREATE NONCLUSTERED INDEX my_index_name ON [$(PROJECT_DB)].[$(PROJECT_SCHEMA)].[tmp_bipolar_multi] (snz_uid);
GO
...
WITH multi_to_add AS (
	...
	FROM [$(PROJECT_DB)].[$(PROJECT_SCHEMA)].[tmp_bipolar_multi] AS m
	WHERE EXISTS (
		SELECT 1
		FROM [$(PROJECT_DB)].[$(PROJECT_SCHEMA)].[tmp_bipolar_solo] AS s
		WHERE m.snz_uid = s.snz_uid
	)
)
SELECT DISTINCT snz_uid, event_date
INTO [$(PROJECT_DB)].[$(PROJECT_SCHEMA)].[defn_mha_bipolar_$(REFRESH)]
FROM (
	SELECT snz_uid, event_date
	FROM [$(PROJECT_DB)].[$(PROJECT_SCHEMA)].[tmp_bipolar_solo]
...
/* Add index */
CREATE NONCLUSTERED INDEX my_index_name ON [$(PROJECT_DB)].[$(PROJECT_SCHEMA)].[defn_mha_bipolar_$(REFRESH)] (snz_uid);
GO
/* Compress final table to save space */
-- original / naive
-- ALTER TABLE [$(PROJECT_DB)].[$(PROJECT_SCHEMA)].[defn_mha_bipolar_$(REFRESH)] REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = PAGE);
-- procedure / faster
EXEC [IDI_UserCode].[$(PROJECT_SCHEMA)].[compress_table_$(PROJECT_DB)] @table = '[$(PROJECT_DB)].[$(PROJECT_SCHEMA)].[defn_mha_bipolar_$(REFRESH)]'
