...
CREATE VIEW [$(PROJECT_SCHEMA)].[max_date_CYF_roc_$(REFRESH)] AS
WITH uid_list AS (
...
count_each_date AS (
	...
	SELECT CAST([cyf_ine_event_from_datetime] AS DATE) AS the_date
		, COUNT(*) AS num
	FROM [IDI_Clean_$(REFRESH)].[cyf_clean].[cyf_intakes_event] i
	LEFT JOIN [IDI_Clean_$(REFRESH)].[cyf_clean].[cyf_intakes_details] d
	ON i.[snz_composite_event_uid] = d.[snz_composite_event_uid]
	WHERE [cyf_ind_cnp_notification_ind] = 'Y'
	AND [cyf_ine_event_from_datetime] BETWEEN DATEADD(YEAR, -5, GETDATE()) AND GETDATE() -- events between now and five years ago
	GROUP BY CAST([cyf_ine_event_from_datetime] AS DATE) -- remove times if DATETIME
