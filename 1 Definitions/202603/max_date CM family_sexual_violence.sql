...
CREATE VIEW [$(PROJECT_SCHEMA)].[max_date_CM_family_sexual_violence_$(REFRESH)] AS
WITH uid_list AS (
...
count_each_date AS (
	...
	SELECT CAST([incident_start_date] AS DATE) AS the_date
		, COUNT(*) AS num
	FROM [IDI_Community].[crim_family_sexual_violence].[family_sexual_violence_$(REFRESH)]
	WHERE [incident_start_date] BETWEEN DATEADD(YEAR, -5, GETDATE()) AND GETDATE() -- events between now and five years ago
	GROUP BY CAST([incident_start_date] AS DATE) -- remove times if DATETIME
