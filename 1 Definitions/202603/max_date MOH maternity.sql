...
DROP VIEW IF EXISTS [$(PROJECT_SCHEMA)].[max_date_MOH_maternity_$(REFRESH)]
GO

CREATE VIEW [$(PROJECT_SCHEMA)].[max_date_MOH_maternity_$(REFRESH)] AS
WITH uid_list AS (
...
count_each_date AS (
	...
	SELECT DATEFROMPARTS([moh_matb_baby_birth_year_nbr],[moh_matb_baby_birth_month_nbr],15) AS the_date
		, COUNT(*) AS num
	FROM [IDI_Clean_$(REFRESH)].[moh_clean].[maternity_baby]
	WHERE DATEFROMPARTS([moh_matb_baby_birth_year_nbr],[moh_matb_baby_birth_month_nbr],15) BETWEEN DATEADD(YEAR, -5, GETDATE()) AND GETDATE() -- events between now and five years ago
	GROUP BY DATEFROMPARTS([moh_matb_baby_birth_year_nbr],[moh_matb_baby_birth_month_nbr],15) 
