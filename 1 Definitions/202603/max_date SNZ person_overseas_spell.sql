...
CREATE VIEW [$(PROJECT_SCHEMA)].[max_date_SNZ_person_overseas_spell_$(REFRESH)] AS
WITH uid_list AS (
...
count_each_date AS (
	...
	SELECT CAST([pos_applied_date] AS DATE) AS the_date
		, COUNT(*) AS num
	FROM [IDI_Clean_$(REFRESH)].[data].person_overseas_spell
	WHERE [pos_applied_date] BETWEEN DATEADD(YEAR, -5, GETDATE()) AND GETDATE() -- events between now and five years ago
	GROUP BY CAST([pos_applied_date] AS DATE) -- remove times if DATETIME
