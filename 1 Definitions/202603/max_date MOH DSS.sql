...
/*
Because the immunisations table does not have a date column, we construct a date value by adding
the target age for an immunisation on the the birth date proxy.

We choose the 8 month immunisations on the assumption that early in a baby's life parents have more
interaction with the health system so these immunisations are more likely to be on time/a better
estimate of the time a baby is 8 months.
*/
...
DROP VIEW IF EXISTS [$(PROJECT_SCHEMA)].[max_date_MOH_DSS_$(REFRESH)]
GO

CREATE VIEW [$(PROJECT_SCHEMA)].[max_date_MOH_DSS_$(REFRESH)] AS
WITH uid_list AS (
...
count_each_date AS (
	...
	SELECT CAST([soc_serv_hist_start_value_date] AS DATE) AS the_date
		, COUNT(*) AS num
	FROM [IDI_Clean_$(REFRESH)].[moh_clean].[socrates_serv_hist]
	WHERE [soc_serv_hist_start_value_date] BETWEEN DATEADD(YEAR, -5, GETDATE()) AND GETDATE() -- events between now and five years ago
	GROUP BY CAST([soc_serv_hist_start_value_date] AS DATE) -- remove times if DATETIME
