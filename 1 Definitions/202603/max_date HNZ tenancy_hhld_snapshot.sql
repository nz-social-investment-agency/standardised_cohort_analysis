...
DROP VIEW IF EXISTS [DL-MAA2023-46].[max_date_HNZ_tenancy_household_snapshot_202506]
GO

CREATE VIEW [DL-MAA2023-46].[max_date_HNZ_tenancy_household_snapshot_202506] AS
WITH uid_list AS (
...
count_each_date AS (
	...
	SELECT CAST([hnz_ths_snapshot_date] AS DATE) AS the_date
		, COUNT(*) AS num
	FROM [IDI_Clean_202506].[hnz_clean].[tenancy_household_snapshot]
	WHERE [hnz_ths_snapshot_date] BETWEEN DATEADD(YEAR, -5, GETDATE()) AND GETDATE() -- events between now and five years ago
	GROUP BY CAST([hnz_ths_snapshot_date] AS DATE) -- remove times if DATETIME
