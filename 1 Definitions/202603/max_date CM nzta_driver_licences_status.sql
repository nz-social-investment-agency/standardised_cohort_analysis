...
CREATE VIEW [$(PROJECT_SCHEMA)].[max_date_CM_nzta_driver_licences_status_$(REFRESH)] AS
WITH uid_list AS (
...
count_each_date AS (
	...
	SELECT CAST([spell_start] AS DATE) AS the_date
		, COUNT(*) AS num
	FROM [IDI_Community].[drv_driver_licences_status].[driver_licences_status_$(REFRESH)]
