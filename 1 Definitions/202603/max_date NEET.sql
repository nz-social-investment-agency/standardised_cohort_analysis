...
DROP VIEW IF EXISTS [DL-MAA2023-46].[max_date_NEET_202506]
GO

CREATE VIEW [DL-MAA2023-46].[max_date_NEET_202506] AS
WITH uid_list AS (

	-- Use all people as NEET is a multi-collection measure
...
count_each_date AS (
	...
	SELECT CAST([moe_enr_prog_start_date] AS DATE) AS the_date
		, COUNT(*) AS num
	FROM [IDI_Clean_202506].[moe_clean].[enrolment]
	WHERE [moe_enr_prog_start_date] BETWEEN DATEADD(YEAR, -5, GETDATE()) AND GETDATE() -- events between now and five years ago
	GROUP BY CAST([moe_enr_prog_start_date] AS DATE) -- remove times if DATETIME
