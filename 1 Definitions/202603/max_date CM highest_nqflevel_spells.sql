...
CREATE VIEW [$(PROJECT_SCHEMA)].[max_date_CM_highest_nqflevel_spells_$(REFRESH)] AS
WITH uid_list AS (
...
count_each_date AS (
	...
	SELECT CAST([nqf_attained_date] AS DATE) AS the_date
		, COUNT(*) AS num
	FROM [IDI_Community].[edu_highest_nqflevel_spells].[highest_nqflevel_spells_$(REFRESH)]
