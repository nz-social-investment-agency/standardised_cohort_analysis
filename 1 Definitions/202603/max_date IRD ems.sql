...
DROP VIEW IF EXISTS [DL-MAA2023-46].[max_date_IRD_ems_202506]
GO

CREATE VIEW [DL-MAA2023-46].[max_date_IRD_ems_202506] AS
WITH uid_list AS (
...
count_each_date AS (
	...
	SELECT CAST(ir_ems_return_period_date AS DATE) AS the_date
		, COUNT(*) AS num
	FROM [IDI_Clean_202506].[ir_clean].[ird_ems]
	WHERE ir_ems_return_period_date BETWEEN DATEADD(YEAR, -5, GETDATE()) AND GETDATE() -- events between now and five years ago
	GROUP BY CAST(ir_ems_return_period_date AS DATE) -- remove times if DATETIME
