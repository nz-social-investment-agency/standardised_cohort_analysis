...
DROP VIEW IF EXISTS [$(PROJECT_SCHEMA)].[max_date_DIA_births_$(REFRESH)]
GO

CREATE VIEW [$(PROJECT_SCHEMA)].[max_date_DIA_births_$(REFRESH)] AS
WITH uid_list AS (
...
count_each_date AS (
	...
	SELECT DATEFROMPARTS([dia_bir_birth_year_nbr],[dia_bir_birth_month_nbr],15) AS the_date
		, COUNT(*) AS num
	FROM [IDI_Clean_$(REFRESH)].[dia_clean].[births]
	WHERE DATEFROMPARTS([dia_bir_birth_year_nbr],[dia_bir_birth_month_nbr],15) BETWEEN DATEADD(YEAR, -5, GETDATE()) AND GETDATE() -- events between now and five years ago
	GROUP BY DATEFROMPARTS([dia_bir_birth_year_nbr],[dia_bir_birth_month_nbr],15)
