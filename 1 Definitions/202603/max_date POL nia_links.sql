...
DROP VIEW IF EXISTS [DL-MAA2023-46].[max_date_POL_nia_links_202506]
GO

CREATE VIEW [DL-MAA2023-46].[max_date_POL_nia_links_202506] AS
WITH uid_list AS (
...
count_each_date AS (
	...
	SELECT CAST([nia_links_rec_date] AS DATE) AS the_date
		, COUNT(*) AS num
	FROM [IDI_Clean_202506].[pol_clean].[nia_links]
	WHERE [nia_links_rec_date] BETWEEN DATEADD(YEAR, -5, GETDATE()) AND GETDATE() -- events between now and five years ago
	GROUP BY CAST([nia_links_rec_date] AS DATE) -- remove times if DATETIME
