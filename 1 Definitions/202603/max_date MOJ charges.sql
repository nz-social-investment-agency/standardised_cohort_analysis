...
DROP VIEW IF EXISTS [DL-MAA2023-46].[max_date_MOJ_charges_202506]
GO

CREATE VIEW [DL-MAA2023-46].[max_date_MOJ_charges_202506] AS
WITH uid_list AS (
...
count_each_date AS (
	...
	SELECT CAST([moj_chg_first_court_hearing_date] AS DATE) AS the_date
		, COUNT(*) AS num
	FROM [IDI_Clean_202506].[moj_clean].[charges]
	WHERE [moj_chg_first_court_hearing_date] BETWEEN DATEADD(YEAR, -5, GETDATE()) AND GETDATE() -- events between now and five years ago
	GROUP BY CAST([moj_chg_first_court_hearing_date] AS DATE) -- remove times if DATETIME
