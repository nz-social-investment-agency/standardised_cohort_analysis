...
count_each_date AS (
	...
	SELECT CAST([period_start_date] AS DATE) AS the_date
		, COUNT(*) AS num
	FROM [IDI_Community].[inc_total_income].[total_income_$(REFRESH)]
...
INTO [$(PROJECT_DB)].[$(PROJECT_SCHEMA)].[max_date_CM_income_t2_total_income_$(REFRESH)]
FROM uid_list
	,max_date
GO

-- Compression
EXEC [IDI_UserCode].[$(PROJECT_SCHEMA)].[compress_table_$(PROJECT_DB)] @table = '[$(PROJECT_DB)].[$(PROJECT_SCHEMA)].[max_date_CM_income_t2_total_income_$(REFRESH)]'
GO

CREATE NONCLUSTERED INDEX i_uid ON [$(PROJECT_DB)].[$(PROJECT_SCHEMA)].[max_date_CM_income_t2_total_income_$(REFRESH)] (snz_uid) INCLUDE (max_date)
GO
