...
CREATE VIEW [$(PROJECT_SCHEMA)].[max_date_CM_OT_findings_$(REFRESH)] AS
WITH uid_list AS (
...
count_each_date AS (
	...
	SELECT CAST(abuse_finding_date AS DATE) AS the_date
		, COUNT(*) AS num
	FROM [IDI_Community].[chld_assess_investig_abuse].[assess_investig_abuse_$(REFRESH)]
	WHERE abuse_finding_date BETWEEN DATEADD(YEAR, -5, GETDATE()) AND GETDATE() -- events between now and five years ago
	GROUP BY CAST(abuse_finding_date AS DATE) -- remove times if DATETIME
