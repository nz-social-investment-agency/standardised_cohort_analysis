...
This max-date is applied to the [NIR] table.
However, we do not use the [NIR] table to make this max date. This is because the [NIR] table does not contain any dates.
So our best estimate is that [nir] table is made from the [nir_event] table. Hence it is appropriate to use the dates in [nir_event]
to deduce the max-date of [nir].
...
CREATE VIEW [$(PROJECT_SCHEMA)].[max_date_MOH_immunisations_$(REFRESH)] AS
WITH uid_list AS (
...
count_each_date AS (
	...
	SELECT CAST([moh_nir_evt_vaccine_date] AS DATE) AS the_date
		, COUNT(*) AS num
	FROM [IDI_Clean_$(REFRESH)].[moh_clean].[nir_event]
	WHERE [moh_nir_evt_vaccine_date] BETWEEN DATEADD(YEAR, -5, GETDATE()) AND GETDATE() -- events between now and five years ago
	GROUP BY CAST([moh_nir_evt_vaccine_date] AS DATE) -- remove times if DATETIME
