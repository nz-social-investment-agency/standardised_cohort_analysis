...
CREATE VIEW [$(PROJECT_SCHEMA)].[max_date_avoid_hosp_$(REFRESH)] AS
WITH uid_list AS (
...
count_each_date AS (
	...
	SELECT CAST([start_date] AS DATE) AS the_date
		, COUNT(*) AS num
	FROM [$(PROJECT_DB)].[$(PROJECT_SCHEMA)].[defn_ASH_PAH_$(REFRESH)]
	WHERE [start_date] BETWEEN DATEADD(YEAR, -5, GETDATE()) AND GETDATE() -- events between now and five years ago
	GROUP BY CAST([start_date] AS DATE) -- remove times if DATETIME
