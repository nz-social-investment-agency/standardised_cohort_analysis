...
DROP VIEW IF EXISTS [DL-MAA2023-46].[max_date_CYF_placements_202506]
GO

CREATE VIEW [DL-MAA2023-46].[max_date_CYF_placements_202506] AS
WITH uid_list AS (
...
count_each_date AS (
	...
	SELECT CAST([cyf_ple_event_from_date_wid_date] AS DATE) AS the_date
		, COUNT(*) AS num
	FROM [IDI_Clean_202506].[cyf_clean].[cyf_placements_event]
	WHERE [cyf_ple_event_from_date_wid_date] BETWEEN DATEADD(YEAR, -5, GETDATE()) AND GETDATE() -- events between now and five years ago
	GROUP BY CAST([cyf_ple_event_from_date_wid_date] AS DATE) -- remove times if DATETIME
