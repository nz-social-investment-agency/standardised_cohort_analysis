...
DROP VIEW IF EXISTS [DL-MAA2023-46].[max_date_CM_nzta_driver_licences_status_202506]
GO

CREATE VIEW [DL-MAA2023-46].[max_date_CM_nzta_driver_licences_status_202506] AS
WITH uid_list AS (
...
count_each_date AS (
	...
	SELECT CAST([spell_start] AS DATE) AS the_date
		, COUNT(*) AS num
	FROM [IDI_Community].[drv_driver_licences_status].[driver_licences_status_202506]
	WHERE [spell_start] BETWEEN DATEADD(YEAR, -5, GETDATE()) AND GETDATE() -- events between now and five years ago
	GROUP BY CAST([spell_start] AS DATE) -- remove times if DATETIME
