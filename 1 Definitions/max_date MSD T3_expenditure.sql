...
DROP VIEW IF EXISTS [DL-MAA2023-46].[max_date_MSD_msd_third_tier_expenditure_202506]
GO

CREATE VIEW [DL-MAA2023-46].[max_date_MSD_msd_third_tier_expenditure_202506] AS
WITH uid_list AS (
...
count_each_date AS (
	...
	SELECT CAST([msd_tte_app_date] AS DATE) AS the_date
		, COUNT(*) AS num
	FROM [IDI_Clean_202506].[msd_clean].[msd_third_tier_expenditure]
	WHERE [msd_tte_app_date] BETWEEN DATEADD(YEAR, -5, GETDATE()) AND GETDATE() -- events between now and five years ago
	GROUP BY CAST([msd_tte_app_date] AS DATE) -- remove times if DATETIME
