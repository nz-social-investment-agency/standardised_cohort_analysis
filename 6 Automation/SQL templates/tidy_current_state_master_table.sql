...
Inputs & Dependencies:
- [{PROJECT_DB}].[{PROJECT_SCHEMA}].[{PREFIX}{COHORT}_current_state_master_table_{REFRESH}]
Output
- [IDI_UserCode].[{PROJECT_SCHEMA}].[{PREFIX}{COHORT}_current_state_master_table_{REFRESH}]
...
CREATE VIEW [{PROJECT_SCHEMA}].[{PREFIX}{COHORT}_current_state_master_table_{REFRESH}] 
AS
SELECT snz_uid
	, organisation
	, client_status
	...
	, IIF(snz_mother_uid IS NOT NULL, 1, NULL) AS has_mother
	, IIF(snz_father_uid IS NOT NULL, 1, NULL) AS has_father
	, IIF(snz_caregiver1_uid IS NOT NULL OR snz_caregiver2_uid IS NOT NULL, 1, NULL) AS has_caregiver
...
--- Demographics ---
	...
	-- We don't want results summarised by sub-national area if genpop, hence set to NULL for summarise tool
	, IIF(client_status = 'genpop', NULL, REGC_current) AS REGC_current
	, IIF(client_status = 'genpop', NULL, REGC_NAME_current) AS REGC_NAME_current
	, IIF(client_status = 'genpop', NULL, TALB_current) AS TALB_current
	, IIF(client_status = 'genpop', NULL, TALB_NAME_current) AS TALB_NAME_current
	, IIF(client_status = 'genpop', NULL, SA3_current) AS SA3_current
	, IIF(client_status = 'genpop', NULL, SA3_NAME_current) AS SA3_NAME_current
	, IIF(client_status = 'genpop', NULL, SA2_current) AS SA2_current
	, IIF(client_status = 'genpop', NULL, SA2_NAME_current) AS SA2_NAME_current
	, CASE 
	...
	-- We don't want results summarised by sub-national area if genpop, hence set to NULL for summarise tool
	, IIF(client_status = 'genpop', NULL, REGC_refdate) AS REGC
	, IIF(client_status = 'genpop', NULL, REGC_NAME_refdate) AS REGC_NAME
	, IIF(client_status = 'genpop', NULL, TALB_refdate) AS TALB
	, IIF(client_status = 'genpop', NULL, TALB_NAME_refdate) AS TALB_NAME
	, IIF(client_status = 'genpop', NULL, SA3_refdate) AS SA3
	, IIF(client_status = 'genpop', NULL, SA3_NAME_refdate) AS SA3_NAME
	, IIF(client_status = 'genpop', NULL, SA2_refdate) AS SA2
	, IIF(client_status = 'genpop', NULL, SA2_NAME_refdate) AS SA2_NAME
	...
	, IIF(res_age_4 = 1,1,NULL) AS res_age_4 --for b4sc
	...
	, IIF(school_attendance_12mo IS NOT NULL, 1, NULL) AS school_enrolled -- for school_attendance_12mo = chroninc absence, regular attendance
...
--- Supporting People in to Work ---
	...
	, CASE
		WHEN main_benefit_duration_caregiver1_12mo > 0 THEN 1
		WHEN main_benefit_duration_caregiver2_12mo > 0 THEN 1
		ELSE NULL END AS main_benefit_caregiver_12mo
	, CASE
		WHEN main_benefit_duration_caregiver1_12mo > 180 THEN 1
		WHEN main_benefit_duration_caregiver2_12mo > 180 THEN 1
		ELSE NULL END AS main_benefit_caregiver_6mo_12mo
	...
	, CASE
		WHEN JSWR_duration_caregiver1_12mo > 0 THEN 1
		WHEN JSWR_duration_caregiver2_12mo > 0 THEN 1
		ELSE NULL END AS JSWR_caregiver_12mo
	, CASE
		WHEN JSWR_duration_caregiver1_12mo > 180 THEN 1
		WHEN JSWR_duration_caregiver2_12mo > 180 THEN 1
		ELSE NULL END AS JSWR_caregiver_6mo_12mo
	...
	, CASE
		WHEN SPS_duration_caregiver1_12mo > 0 THEN 1
		WHEN SPS_duration_caregiver2_12mo > 0 THEN 1
		ELSE NULL END AS SPS_caregiver_12mo
	, CASE
		WHEN SPS_duration_caregiver1_12mo > 180 THEN 1
		WHEN SPS_duration_caregiver2_12mo > 180 THEN 1
		ELSE NULL END AS SPS_caregiver_6mo_12mo
	...
	, CASE
		WHEN employed_duration_caregiver1_12mo > 0 THEN 1
		WHEN employed_duration_caregiver2_12mo > 0 THEN 1
		ELSE NULL END AS employed_caregiver_12mo
	, CASE
		WHEN employed_duration_caregiver1_12mo > 180 THEN 1
		WHEN employed_duration_caregiver2_12mo > 180 THEN 1
		ELSE NULL END AS employed_caregiver_6mo_12mo
	...
	, CASE
		WHEN dl_holder_full_caregiver1_current > 0 THEN dl_holder_full_caregiver1_current
		WHEN dl_holder_full_caregiver2_current > 0 THEN dl_holder_full_caregiver2_current
		ELSE NULL END AS dl_holder_full_caregiver_current
	, CASE
		WHEN dl_holder_caregiver1_current > 0 THEN dl_holder_caregiver1_current
		WHEN dl_holder_caregiver2_current > 0 THEN dl_holder_caregiver2_current
		ELSE NULL END AS dl_holder_caregiver_current
	...
	, CASE
		WHEN SNG_basics_caregiver1_12mo > 0 THEN 1
		WHEN SNG_basics_caregiver2_12mo > 0 THEN 1
		ELSE NULL END AS SNG_basics_caregiver_12mo
	...
	, CASE
		WHEN SNG_food_caregiver1_12mo > 0 THEN 1
		WHEN SNG_food_caregiver2_12mo > 0 THEN 1
		ELSE NULL END AS SNG_food_caregiver_12mo
...
--- Improved Health ---
	...
	, CASE
		WHEN Hospitalisations_caregiver1_12mo > 0 THEN 1
		WHEN Hospitalisations_caregiver2_12mo > 0 THEN 1
		ELSE NULL END AS Hospitalisations_caregiver_12mo

	, IIF(DSS_12mo > 0,1,NULL) AS DSS_12mo
	, IIF(DSS_mother_12mo > 0,1,NULL) AS DSS_mother_12mo
	, IIF(DSS_father_12mo > 0,1,NULL) AS DSS_father_12mo
	, CASE
		WHEN DSS_caregiver1_12mo > 0 THEN DSS_caregiver1_12mo
		WHEN DSS_caregiver2_12mo > 0 THEN DSS_caregiver2_12mo
		ELSE NULL END AS DSS_caregiver_12mo

	, IIF(DSS_ever > 0,1,NULL) AS DSS_ever
	, IIF(DSS_mother_ever > 0,1,NULL) AS DSS_mother_ever
	, IIF(DSS_father_ever > 0,1,NULL) AS DSS_father_ever
	, CASE
		WHEN DSS_caregiver1_ever > 0 THEN DSS_caregiver1_ever
		WHEN DSS_caregiver2_ever > 0 THEN DSS_caregiver2_ever
		ELSE NULL END AS DSS_caregiver_ever
	...
	, CASE
		WHEN pharmaceutical_drugs_caregiver1_12mo > 0 THEN 1
		WHEN pharmaceutical_drugs_caregiver2_12mo > 0 THEN 1
		ELSE NULL END AS pharmaceutical_drugs_caregiver_12mo

	, IIF(had_b4sc > 0 , 1, NULL) as had_b4sc
	...
	, CASE
		WHEN pho_enrolment_caregiver1_current > 0 THEN pho_enrolment_caregiver1_current
		WHEN pho_enrolment_caregiver2_current > 0 THEN pho_enrolment_caregiver2_current
		ELSE NULL END AS pho_enrolment_caregiver_current
	...
	, CASE
		WHEN acc_injuries_caregiver1_12mo > 0 THEN acc_injuries_caregiver1_12mo
		WHEN acc_injuries_caregiver2_12mo > 0 THEN acc_injuries_caregiver2_12mo
		ELSE NULL END AS acc_injuries_caregiver_12mo
	...
	, CASE
		WHEN acc_supports_caregiver1_12mo > 0 THEN acc_supports_caregiver1_12mo
		WHEN acc_supports_caregiver2_12mo > 0 THEN acc_supports_caregiver2_12mo
		ELSE NULL END AS acc_supports_caregiver_12mo
	...
	, CASE
		WHEN MHA_service_caregiver1_ever > 0 THEN MHA_service_caregiver1_ever
		WHEN MHA_service_caregiver2_ever > 0 THEN MHA_service_caregiver2_ever
		ELSE NULL END AS MHA_service_caregiver_ever
	, COALESCE(MHA_entity_caregiver1_ever__min, MHA_entity_caregiver1_ever__min) AS MHA_entity_caregiver_ever__min
	, COALESCE(MHA_entity_caregiver2_ever__max, MHA_entity_caregiver2_ever__max) AS MHA_entity_caregiver_ever__max
	...
	, CASE
		WHEN MHA_service_caregiver1_12mo > 0 THEN MHA_service_caregiver1_12mo
		WHEN MHA_service_caregiver2_12mo > 0 THEN MHA_service_caregiver2_12mo
		ELSE NULL END AS MHA_service_caregiver_12mo
	, COALESCE(MHA_entity_caregiver1_12mo__min, MHA_entity_caregiver1_12mo__min) AS MHA_entity_caregiver_12mo__min
	, COALESCE(MHA_entity_caregiver2_12mo__max, MHA_entity_caregiver2_12mo__max) AS MHA_entity_caregiver_12mo__max
	...
	, CASE
		WHEN ED_visits_caregiver1_12mo > 0 THEN 1
		WHEN ED_visits_caregiver2_12mo > 0 THEN 1
		ELSE NULL END AS ED_visits_caregiver_12mo
	...
	, CASE
		WHEN avoid_hosp_ash_pah_caregiver1_12mo > 0 THEN 1
		WHEN avoid_hosp_ash_pah_caregiver2_12mo > 0 THEN 1
		ELSE NULL END AS avoid_hosp_ash_pah_caregiver_12mo
...
--- Growing Income and Wealth ---
	...
	, CASE
		WHEN ROUND(income_caregiver1_12mo, 2) < 68120.00 THEN 1
		WHEN ROUND(income_caregiver2_12mo, 2) < 68120.00 THEN 1
		ELSE NULL END AS below_med_income_caregiver_12mo
...
--- Greater Safety ---
	...
	, CASE
		WHEN crime_victimisations_caregiver1_12mo > 0 THEN crime_victimisations_caregiver1_12mo
		WHEN crime_victimisations_caregiver2_12mo > 0 THEN crime_victimisations_caregiver2_12mo
		ELSE NULL END AS crime_victimisations_caregiver_12mo
	...
	, CASE
		WHEN violent_crime_victimisations_caregiver1_12mo > 0 THEN violent_crime_victimisations_caregiver1_12mo
		WHEN violent_crime_victimisations_caregiver2_12mo > 0 THEN violent_crime_victimisations_caregiver2_12mo
		ELSE NULL END AS violent_crime_victimisations_caregiver_12mo
	...
	, CASE
		WHEN FVSV_victim_caregiver1_ever > 0 THEN FVSV_victim_caregiver1_ever
		WHEN FVSV_victim_caregiver2_ever > 0 THEN FVSV_victim_caregiver2_ever
		ELSE NULL END AS FVSV_victim_caregiver_ever
	...
	, CASE
		WHEN FVSV_victim_caregiver1_12mo > 0 THEN 1
		WHEN FVSV_victim_caregiver2_12mo > 0 THEN 1
		ELSE NULL END AS FVSV_victim_caregiver_12mo
	...
	, CASE
		WHEN alcohol_offences_caregiver1_12mo > 0 THEN alcohol_offences_caregiver1_12mo
		WHEN alcohol_offences_caregiver2_12mo > 0 THEN alcohol_offences_caregiver2_12mo
		ELSE NULL END AS alcohol_offences_caregiver_12mo
	, CASE
		WHEN violent_offences_caregiver1_12mo > 0 THEN violent_offences_caregiver1_12mo
		WHEN violent_offences_caregiver2_12mo > 0 THEN violent_offences_caregiver2_12mo
		ELSE NULL END AS violent_offences_caregiver_12mo
	, CASE
		WHEN non_violent_offences_caregiver1_12mo > 0 THEN non_violent_offences_caregiver1_12mo
		WHEN non_violent_offences_caregiver2_12mo > 0 THEN non_violent_offences_caregiver2_12mo
		ELSE NULL END AS non_violent_offences_caregiver_12mo
	, CASE
		WHEN drug_offences_caregiver1_12mo > 0 THEN drug_offences_caregiver1_12mo
		WHEN drug_offences_caregiver2_12mo > 0 THEN drug_offences_caregiver2_12mo
		ELSE NULL END AS drug_offences_caregiver_12mo
	...
	, CASE
		WHEN police_interactions_any_caregiver1_12mo > 0 THEN police_interactions_any_caregiver1_12mo
		WHEN police_interactions_any_caregiver2_12mo > 0 THEN police_interactions_any_caregiver2_12mo
		ELSE NULL END AS police_interactions_any_caregiver_12mo
	...
	, CASE
		WHEN court_charges_caregiver1_ever > 0 THEN court_charges_caregiver1_ever
		WHEN court_charges_caregiver2_ever > 0 THEN court_charges_caregiver2_ever
		ELSE NULL END AS court_charges_caregiver_ever
	...
	, CASE
		WHEN court_charges_caregiver1_12mo > 0 THEN 1
		WHEN court_charges_caregiver2_12mo > 0 THEN 1
		ELSE NULL END AS court_charges_caregiver_12mo
	...
	, CASE
		WHEN corr_any_caregiver1_12mo > 0 THEN corr_any_caregiver2_12mo
		WHEN corr_any_caregiver2_12mo > 0 THEN corr_any_caregiver1_12mo
		ELSE NULL END AS corr_any_caregiver_12mo
	...
	, CASE	
		WHEN corr_incarcerated_caregiver1_ever > 0 THEN corr_incarcerated_caregiver1_ever
		WHEN corr_incarcerated_caregiver2_ever > 0 THEN corr_incarcerated_caregiver2_ever
		ELSE NULL END AS corr_incarcerated_caregiver_ever
	...
	, CASE
		WHEN corr_incarcerated_caregiver1_12mo > 0 THEN 1
		WHEN corr_incarcerated_caregiver2_12mo > 0 THEN 1
		ELSE NULL END AS corr_incarcerated_caregiver_12mo
...
--- Ensuring secure and stable housing ---
	...
	, CASE
		WHEN hnz_waitlist_caregiver1_12mo> 0 THEN hnz_waitlist_caregiver1_12mo
		WHEN hnz_waitlist_caregiver2_12mo> 0 THEN hnz_waitlist_caregiver2_12mo
		ELSE NULL END AS hnz_waitlist_caregiver_12mo
	...
	, CASE
		WHEN social_housing_caregiver1_12mo> 0 THEN social_housing_caregiver1_12mo
		WHEN social_housing_caregiver2_12mo> 0 THEN social_housing_caregiver2_12mo
		ELSE NULL END AS social_housing_caregiver_12mo
	...
	, CASE
		WHEN emergency_housing_caregiver1_12mo > 0 THEN emergency_housing_caregiver1_12mo
		WHEN emergency_housing_caregiver2_12mo > 0 THEN emergency_housing_caregiver2_12mo
		ELSE NULL END AS emergency_housing_caregiver_12mo
...
--- Improving Knowledge and Skills ---
	...
	, IIF(highest_qual_caregiver1_current > 0 OR highest_qual_caregiver2_current > 0, 1, NULL) AS highest_qual_caregiver_current
	, COALESCE(highest_qual_entity_caregiver1__min, highest_qual_entity_caregiver2__min) AS highest_qual_entity_caregiver__min
	, COALESCE(highest_qual_entity_caregiver1__max, highest_qual_entity_caregiver2__max) AS highest_qual_entity_caregiver__max
...
FROM [{PROJECT_DB}].[{PROJECT_SCHEMA}].[{PREFIX}{COHORT}_current_state_master_table_{REFRESH}]
GO
