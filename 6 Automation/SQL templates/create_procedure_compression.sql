...
Example use:
EXEC [IDI_UserCode].[{PROJECT_SCHEMA}].[compress_table] @table = '[{PROJECT_DB}].[{PROJECT_SCHEMA}].[non_neet_spells]'

To keep the original and compressed table:
EXEC [IDI_UserCode].[{PROJECT_SCHEMA}].[compress_table] @table = '[{PROJECT_DB}].[{PROJECT_SCHEMA}].[non_neet_spells]', @swap_tables = 0

To just review the commands that will be executed:
EXEC [IDI_UserCode].[{PROJECT_SCHEMA}].[compress_table] @table = '[{PROJECT_DB}].[{PROJECT_SCHEMA}].[non_neet_spells]', @debug = 1
*/

USE IDI_UserCode
GO

DROP PROCEDURE IF EXISTS [{PROJECT_SCHEMA}].[compress_table_{PROJECT_DB}]
GO

CREATE PROCEDURE [{PROJECT_SCHEMA}].[compress_table_{PROJECT_DB}]
...
BEGIN
	...
	SELECT i.name
		, i.is_unique
		, i.is_primary_key
	FROM {PROJECT_DB}.sys.indexes AS i
	INNER JOIN {PROJECT_DB}.sys.objects AS o
	ON i.object_id = o.object_id
	INNER JOIN {PROJECT_DB}.sys.schemas AS s
	...
	BEGIN
		...
		FROM {PROJECT_DB}.sys.index_columns AS ic
		INNER JOIN {PROJECT_DB}.sys.columns AS c
		ON ic.object_id = c.object_id
		AND ic.column_id = c.column_id
		WHERE ic.object_id = OBJECT_ID(@tbl_full)
		AND ic.index_id = (
			SELECT index_id
			FROM {PROJECT_DB}.sys.indexes
	...
	BEGIN
		BEGIN TRANSACTION T1;
			...
			-- rename new table
			SET @sql = 'EXEC {PROJECT_DB}.dbo.sp_rename ''[@schema@].[@new_tbl@]'', ''@tbl@'''
