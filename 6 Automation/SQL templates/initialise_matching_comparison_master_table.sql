...
- [{PROJECT_DB}].[{PROJECT_SCHEMA}].[{PREFIX}{COHORT}_possible_client_matches_{REFRESH}]
...
CREATE TABLE [{PROJECT_DB}].[{PROJECT_SCHEMA}].[{PREFIX}{COHORT}_comparison_matching_master_table_{REFRESH}] (
...
FROM [{PROJECT_DB}].[{PROJECT_SCHEMA}].[{PREFIX}{COHORT}_possible_client_matches_{REFRESH}] AS m
...
IF (
	...
	AND
	(SELECT COUNT(*) FROM [{PROJECT_DB}].[{PROJECT_SCHEMA}].[{PREFIX}{COHORT}_client_{REFRESH}]) >= 10
)
BEGIN -- startconditional population of table
	...
	INSERT INTO [{PROJECT_DB}].[{PROJECT_SCHEMA}].[{PREFIX}{COHORT}_comparison_matching_master_table_{REFRESH}] (snz_uid, reference_date, matching_date, age_ref_date)
	SELECT snz_uid
	...
	DELETE FROM [{PROJECT_DB}].[{PROJECT_SCHEMA}].[{PREFIX}{COHORT}_comparison_matching_master_table_{REFRESH}]
	WHERE snz_uid IN (SELECT snz_uid FROM [{PROJECT_DB}].[{PROJECT_SCHEMA}].[{PREFIX}{COHORT}_client_{REFRESH}])
	
	-------------------------------------------------------------------------------
	-- Insert records to match

	INSERT INTO [{PROJECT_DB}].[{PROJECT_SCHEMA}].[{PREFIX}{COHORT}_comparison_matching_master_table_{REFRESH}] (snz_uid, reference_date, matching_date, age_ref_date, to_match)
	SELECT c.snz_uid
		,reference_date
		-- end of previous quarter
		,CASE
			WHEN MONTH(c.reference_date) IN (01, 02, 03) THEN DATEFROMPARTS(YEAR(c.reference_date) - 1, 12, 31)
			WHEN MONTH(c.reference_date) IN (04, 05, 06) THEN DATEFROMPARTS(YEAR(c.reference_date), 03, 31)
			WHEN MONTH(c.reference_date) IN (07, 08, 09) THEN DATEFROMPARTS(YEAR(c.reference_date), 06, 30)
			WHEN MONTH(c.reference_date) IN (10, 11, 12) THEN DATEFROMPARTS(YEAR(c.reference_date), 09, 30)
			END AS matching_date
		,DATEDIFF(MONTH, p.snz_birth_date_proxy, c.reference_date) / 12 AS age_ref_date
		,1 AS to_match
	FROM [{PROJECT_DB}].[{PROJECT_SCHEMA}].[{PREFIX}{COHORT}_client_{REFRESH}] AS c
	INNER JOIN [IDI_Clean_{REFRESH}].[data].[personal_detail] AS p
...
CREATE NONCLUSTERED INDEX i_uid ON [{PROJECT_DB}].[{PROJECT_SCHEMA}].[{PREFIX}{COHORT}_comparison_matching_master_table_{REFRESH}] (snz_uid)
GO
...
BEGIN 
	...
	UPDATE [{PROJECT_DB}].[{PROJECT_SCHEMA}].[{PREFIX}{COHORT}_comparison_matching_master_table_{REFRESH}]
	SET 
...
BEGIN 
	...
	FROM [{PROJECT_DB}].[{PROJECT_SCHEMA}].[{PREFIX}{COHORT}_comparison_matching_master_table_{REFRESH}] AS mt
	INNER JOIN [IDI_Clean_{REFRESH}].[data].[personal_detail] AS p
...
DROP VIEW IF EXISTS [{PROJECT_SCHEMA}].[{PREFIX}{COHORT}_tmp_helper_{REFRESH}]
GO
DROP INDEX i_uid ON [{PROJECT_DB}].[{PROJECT_SCHEMA}].[{PREFIX}{COHORT}_comparison_matching_master_table_{REFRESH}]
GO

-------------------------------------------------------------------------------
-- Index

CREATE NONCLUSTERED INDEX i_uid_std_date ON [{PROJECT_DB}].[{PROJECT_SCHEMA}].[{PREFIX}{COHORT}_comparison_matching_master_table_{REFRESH}] (snz_uid, std_start_date, std_end_date)
CREATE NONCLUSTERED INDEX i_uid_lag_date ON [{PROJECT_DB}].[{PROJECT_SCHEMA}].[{PREFIX}{COHORT}_comparison_matching_master_table_{REFRESH}] (snz_uid, lag_start_date, lag_end_date)
CREATE NONCLUSTERED INDEX i_references	 ON [{PROJECT_DB}].[{PROJECT_SCHEMA}].[{PREFIX}{COHORT}_comparison_matching_master_table_{REFRESH}] (age_ref_date, reference_date)
GO
