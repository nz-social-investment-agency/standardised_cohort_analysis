...
settings_file = glue::glue("2 Analysis/TePuawaitanga/202603/Execution/in_progress_settings.RDS")
...
if(is_current_state){
  ...
  mapping_csv = read.csv(glue::glue("6 Automation/Mappings/mapping - dimension1.csv"))
  df = df |>
    dplyr::left_join(mapping_csv, by = c("dim1_type", "dim1_code"))
  
  mapping_csv = read.csv(glue::glue("6 Automation/Mappings/mapping - dimension2.csv"))
  df = df |>
...
if(is_current_state){
  ...
  metadata_df = data.frame(
    organisation_name = rep(TePuawaitanga, length(values)),
...
#remove GenPop values other than country
  
  df = df |>
    dplyr::filter(
      !(client_status == 'genpop' &
          dimension1_type != 'country' | 
          client_status == 'genpop' &
          indicator == 'Population' | #drop dimension rows
          client_status == 'genpop' &
          period == 'current' #drop genpop 'current' rows
      )
    )
  
  df = df |>
    dplyr::filter(
      !(dimension2_type == 'Age group - Reference'
      )
    )
...
if(is_time_series){
  df = dplyr::filter(df, !is.na(conf_count_w_indicator|!is.na(conf_annualised_total)))
}
